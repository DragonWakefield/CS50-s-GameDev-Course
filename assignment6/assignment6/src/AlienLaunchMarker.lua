...
function AlienLaunchMarker:update(dt)
    ...
    elseif self.launched then
        if love.keyboard.wasPressed('space') and not self.split and not self.hit then
            self.split = true
            local x = self.alien[1].body:getX()
            local y = self.alien[1].body:getY()

            --self.alien[1].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, -(y ))

            self.alien[2] = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY(), 'Player')

            self.alien[2].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, -(y+ 25))

            -- make the alien pretty bouncy
            self.alien[2].fixture:setRestitution(0.4)
            self.alien[2].body:setAngularDamping(1)

           

            self.alien[3] = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY(), 'Player')
            self.alien[3].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, -(y-25))

            -- make the alien pretty bouncy
            self.alien[3].fixture:setRestitution(0.4)
            self.alien[3].body:setAngularDamping(1)
        end 
    end
end
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        for i = 1, #self.alien do
            if not self.alien[i].body:isDestroyed() then
                self.alien[i]:render()
            end 
        end
    end
