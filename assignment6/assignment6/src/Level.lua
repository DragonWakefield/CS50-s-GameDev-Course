...
function Level:init()
    ...
    self.edgeShape2 = love.physics.newEdgeShape(VIRTUAL_WIDTH,0,VIRTUAL_WIDTH,VIRTUAL_HEIGHT)
    ...
    self.limitBody = love.physics.newBody(self.world,VIRTUAL_HEIGHT,0,'static')
    self.limitFixture = love.physics.newFixture(self.limitBody, self.edgeShape2)
    self.limitFixture:setFriction(0.5)
    self.limitFixture:setUserData('Invis_wall')
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        for i = 1, #self.launchMarker.alien do
            if not self.launchMarker.alien[i].body:isDestroyed() then
