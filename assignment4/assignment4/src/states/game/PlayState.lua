...
function PlayState:init()
    ...
    self.x = 0 
    self.y =0
    local pit_fall = false


    while not pit_fall do
        local map_Checker = self:checkMap(i)
        if map_Checker then
            self.x = self.x + 16
            i = i + 1
...
function PlayState:enter(params)
    self.camX = 0
    self.camY = 0
    self.level = LevelMaker.generate(params.level_width, 10)
    self.tileMap = self.level.tileMap
    self.background = math.random(3)
    self.backgroundX = 0
...
function PlayState:checkMap(x) 

    if self.tileMap.tiles[7][x+1].id == TILE_ID_EMPTY then
        return true
    end 
